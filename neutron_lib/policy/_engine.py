...
# Set by the service to route server-side computed policy to its own policy
# engine. Takes (rule, credentials) and returns a bool verdict, or None to
# defer to oslo.policy. These rules resolve on _ROLE_ENFORCER, which the
# service does not own, so the seam has to live here.
rego_hook = None
